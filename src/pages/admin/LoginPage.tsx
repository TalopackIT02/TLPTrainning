...
export default function LoginPage() {
  const auth = useAuth(); const location = useLocation(); const [email, setEmail] = useState(auth.demoMode ? 'admin@tlp.demo' : ''); const [password, setPassword] = useState(auth.demoMode ? 'demo1234' : ''); const [error, setError] = useState(''); const [busy, setBusy] = useState(false)
  ...
  return <main className="grid min-h-screen place-items-center bg-slate-100 p-4"><Card className="w-full max-w-md"><CardHeader className="items-center text-center"><div className="mb-2 grid size-12 place-items-center rounded-lg bg-primary text-primary-foreground"><GraduationCap /></div><CardTitle className="text-2xl">TLP Training</CardTitle><CardDescription>Đăng nhập dành cho quản trị viên / người đào tạo</CardDescription></CardHeader><CardContent><form className="flex flex-col gap-4" onSubmit={submit}><label className="field"><span className="field-label">Email</span><Input type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" /></label><label className="field"><span className="field-label">Mật khẩu</span><Input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" /></label>{error ? <p className="text-sm text-destructive" role="alert">{error}</p> : null}<Button size="lg" disabled={busy}>{busy ? <LoaderCircle data-icon="inline-start" className="animate-spin" /> : null}Đăng nhập</Button>{auth.demoMode ? <p className="text-center text-xs text-muted-foreground">Chế độ demo local — dữ liệu được lưu trong trình duyệt.</p> : null}</form></CardContent></Card></main>
}
