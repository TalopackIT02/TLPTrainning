...
const isInviteCallback = () => /type=(invite|signup|magiclink)/.test(window.location.hash)
const enterAdminAfterInvite = () => { if (isInviteCallback()) window.setTimeout(() => { window.location.hash = '/admin' }, 0) }
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { if (!supabase) return; supabase.auth.getSession().then(({ data }) => { setSession(data.session); if (data.session) enterAdminAfterInvite(); setLoading(false) }); const { data } = supabase.auth.onAuthStateChange((event, next) => { setSession(next); if (event === 'PASSWORD_RECOVERY') setPasswordRecovery(true); if (event === 'SIGNED_IN') enterAdminAfterInvite() }); return () => data.subscription.unsubscribe() }, [])
